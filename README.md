...
* **Bayesian Inference:** Use of JAGS for full posterior estimation.
* **Nonparametric Methods:** Application of Dirichlet Process Mixture Models for flexible distribution modeling.
* **Rigor:** Model validation via **Transformed Residual Analysis** (Exponentiality Test).

## 📄 Files
* `Hawkes_Process_Bayesian_Modeling.R`: Core script for MLE, Bayesian estimation (JAGS), and Dirichlet Process modeling.
* `Hawkes_Process_Diagnostic_Analysis.R`: Dedicated script for rigorous residual analysis (QQ-Plots, K-S Test) to validate the model fit.
